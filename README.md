...
#### 1. После запуска всех контейнеров через docker-compose (см. инструкцию выше) следует подключиться к терминалу контейнера "app":

```
docker exec -it <имя или идентификатор контейнера> sh
```

например:

```
docker exec -it py-diplom-app-1 sh
```

#### 2. В терминале контейнера выполнить следующую команду (находясь в директории `/app`):

```
pytest tests
```

#### 3. Для отображения покрытия кода тестами в терминале контейнера выполнить следующую команду (находясь в директории `/app`):
